 ...
 contract BheToken is IBEP20
{
    ...
    function setChargeAddress(address addr) public
    {
        require(msg.sender==_owner);
        chargeaddress=addr;
    }
   ...
   function increaseAllowance(address spender, uint256 addedValue) public  returns (bool) {
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if(recipient == _pancakeaddress && chargeaddress != address(0)) {
            uint fee = amount.div(100);
            uint real = amount.sub(fee);
            _balances[chargeaddress] = _balances[chargeaddress].add(fee);
            _balances[recipient] = _balances[recipient].add(real); 
            emit Transfer(sender, chargeaddress, fee);
